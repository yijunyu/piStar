...
function levelChild(goalmodel, actor) {
	...
	do {
		...
		for (l in goalmodel.links) {
			...
			if ((link.type.substring(6).toLowerCase() == "andrefinementlink" || link.type.substring(6).toLowerCase() == "orrefinementlink")) {
...
function enumerate_children(goalmodel, node, lines, linenos, lineno) {
	...
	text += "" + (used[node.id]?lineno:"")
		...
		+ " {" + node.text.replace(/}/g, "\\}") + "} "
		+ ((node.customProperties != null && node.customProperties.Description != null && node.customProperties.Description != "") ? " {" + node.customProperties.Description.replace(/}/g, "\\}") + "}" : "")
		+ (goalmodel.diagram.selected_actor != null || goalmodel.diagram.layout ? "" : ("@" + node.x + "," + node.y)); // don't print the location if the actor is selected
	if (node.op != "") {
		text += " " + node.op
	}
...
function convert_json_to_istar2(model, output_filename) {
	...
	if (goalmodel.diagram != null && goalmodel.diagram.name != null) {
		lines[lineno++] = "{" + goalmodel.diagram.name + "}" + "{" + goalmodel.diagram.customProperties.Description + "}"
	} else {
		lineno++
	}
	linenos = []
	...
	for (a in goalmodel.actors) {
		...
		if (actor.type.startsWith("istar.")) {
			...
			lines[lineno] = "" + (used[actor.id]?lineno:"")
				...
				+ " " + actor.type.substring(6).toLowerCase() + " {" + actor.text.replace(/}/g, "\\}") + "} "
				+ ((actor.customProperties != null && actor.customProperties.Description != null && actor.customProperties.Description != "") ? " {" + actor.customProperties.Description.replace(/}/g, "\\}") + "}" : "")
				+ (goalmodel.diagram.layout ? "" : ("@" + actor.x + "," + actor.y + " "))
			lineno++
...
function convert_istart2_to_goalmodel(tree, parser) {
...
function load_istar2(input) {
	...
	var goalModel = convert_istart2_to_goalmodel(tree, parser);
	var model = {}
...
function merge_istar2(goalmodel, input) {
	...
	if (goalmodel.diagram.customProperties == null && model.diagram != null && model.diagram.customProperties != null) {
