    ...
    if (added_dependums[dependency.text] == null) {
      ...
      if (goalModel.display != null && goalModel.display[dependency.id] != null) {
        g.setNode(dependency.id, {
          ...
          width: goalModel.display[dependency.id].width,
          height: goalModel.display[dependency.id].height
        });
      }
    }
  ...
  g.nodes().forEach(function (v) {
    ...
    goalModel.actors.forEach(function (actor) {
      if (actor.id == v && node != null) {
        actor.x = node.x - goalModel.display[actor.id].width / 2 + 1;
        actor.y = node.y - goalModel.display[actor.id].height / 2 + 1;
    ...
    goalModel.dependencies.forEach(function (dependency) {
      if (added_dependums[dependency.text] == v && node != null) {
        dependency.x = node.x - goalModel.display[dependency.id].width / 2 + 1;
        dependency.y = node.y - goalModel.display[dependency.id].height / 2 + 1;
